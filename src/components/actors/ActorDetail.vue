<template>
  <div>
    <div
      class="container mx-auto px-4 py-16 flex flex-col md:flex-row lg:flex-row"
    >
      <div class="flex-none">
      ...
      <div class="md:ml-20 lg:ml-20">
        <h2 class="text-4xl font-semibold pb-2 pt-2">
          {{ this.actor.name }}
        </h2>
  ...
  methods: {
    async fetchActor(actorId) {
      ...
      const response = await this.$http.get(
        "https://api.themoviedb.org/3/person/" + actorId + `?api_key=${token}`
      );
    ...
    async fetchCredits(actorId) {
      ...
      const response = await this.$http.get(
        "https://api.themoviedb.org/3/person/" +
          actorId +
          `/combined_credits?api_key=${token}`
      );
